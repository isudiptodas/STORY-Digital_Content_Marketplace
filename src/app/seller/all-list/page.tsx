      ...
      <div className={`w-full flex flex-col justify-start items-center relative overflow-hidden`}>
        ...
        <div className={`w-full px-4 flex flex-wrap justify-start items-center gap-4 relative overflow-hidden py-5 lg:pt-8 rounded-lg mt-3`}>
          {allProducts.length > 0 && allProducts.map((prod, index) => {
            return <div key={index} onClick={() => {navigate(prod.id)}} className="w-full md:w-[40%] lg:w-[30%] h-auto cursor-pointer hover:scale-95 duration-200 ease-in-out shadow-lg overflow-hidden rounded-lg xl:rounded-xl flex flex-col py-2 px-2 justify-start items-center">
              ...
              <div className={`w-full flex justify-between items-center gap-2 mt-3`}>
                ...
                <span className={`w-full py-2 rounded-lg bg-red-500 text-white font-Kanit flex justify-center items-center gap-2 hover:bg-red-700 duration-200 ease-in-out active:scale-95`}><IoMdRemoveCircle /> Unlist</span>
              </div>
            </div>
          })}
        </div>
